...
- **Author**: ngxccc
- **Status**: Implemented & Verified (All 3 Milestones Completed)
- **Target Component**: `internal/controller.go`, `internal/window.go`, `internal/db.go`, `internal/article.go`
...
| Scenario / Archive Size | Baseline (Full Rebuild) | Milestone 1 (In-Place Mutation) | Milestones 2 & 3 (Async Queue + Preview Cache) | Total Speedup Factor |
| :--- | :--- | :--- | :--- | :--- |
| **100 Articles - Latency** | `540.7 μs/op` | `94.0 μs/op` | **`24.1 μs/op`** | **$22.4\times$ faster** |
| **100 Articles - Memory** | `440.6 KB/op` | `15.9 KB/op` | **`15.4 KB/op`** | **$-96.5\%$ RAM** |
...
| **500 Articles - Memory** | `1,838.3 KB/op` | `16.0 KB/op` | **`15.5 KB/op`** | **$-99.15\%$ RAM** |
...
|         ├───────────────────────────────┐                               |
+─────────┼───────────────────────────────┼───────────────────────────────+
          │ (Non-blocking Channel Send)   │ (Cache Lookup: a.preview)
          ▼                               ▼
+──────────────────────────────+  +───────────────────────────────────────+
|   Background DB Worker       |  |       Preview Cache Engine            |
|                              |  |                                       |
| [Bounded Channel (chan int)] |  | [Lazy Memoization in Article.preview] |
|         │                    |  |   - First visit: html2text once       |
|   (200ms Debounce Batch)     |  |   - Subsequent: O(1) instant return   |
|         ▼                    |  +───────────────────────────────────────+
| [Batch SQLite UPDATE]        |
|  "WHERE id IN (?, ?, ...)"   |
+──────────────────────────────+
...
* `DB.Close()` flushes in-flight queue items and closes the SQLite connection cleanly with `workerDone` synchronization.

### 3.3. Preview Cache & Lazy Rendering (`internal/article.go` & `internal/window.go`) - COMPLETED
* `preview string` in `Article` struct.
* `AddPreview()` memoizes parsed plain text to avoid redundant regex passes.

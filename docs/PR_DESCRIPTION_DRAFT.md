# Pull Request: Eliminate O(N) Table Allocation Storm, Defer Disk I/O, and Memoize Previews on Navigation

## 1. Summary & Problem Statement

When navigating articles rapidly using keybindings (`w`/`s`, `j`/`k`, or arrow keys) in feeds containing hundreds or thousands of articles, `gorss` experienced progressive UI latency degradation, stuttering, and visible frame drops.

Profiling and benchmarking on an AMD Ryzen 5 system demonstrated that each single article selection step previously triggered an **$O(N)$ UI table tear-down and full rebuild**, allocating tens of thousands of temporary heap objects per second and blocking the UI thread with synchronous SQLite disk transactions and regex-heavy HTML parsing passes.

---

## 2. Benchmark Measurements (Full Optimization Progression)

Measured using `go test -bench=BenchmarkSelectArticle -benchmem` on **AMD Ryzen 5 PRO 6650U (Go 1.25)**:

| Archive Size | Metric | Baseline (Original) | Milestone 1 (In-Place Mutation) | Milestone 2 & 3 (Async Queue + Preview Cache) | Total Improvement |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **100 Articles** | **Latency** | `540.7 μs/op` | `94.0 μs/op` | **`24.1 μs/op`** | **$22.4\times$ faster** |
| | **Heap Memory** | `440.6 KB/op` | `15.9 KB/op` | **`15.4 KB/op`** | **$-96.5\%$ memory** |
| | **Allocations** | `5,460 allocs/op` | `166 allocs/op` | **`152 allocs/op`** | **$-97.2\%$ allocs** |
| **500 Articles** | **Latency** | `2,258.5 μs/op` | `110.5 μs/op` | **`31.9 μs/op`** | **$70.8\times$ faster** |
| | **Heap Memory** | `1,838.3 KB/op` | `16.0 KB/op` | **`15.5 KB/op`** | **$-99.15\%$ memory** |
| | **Allocations** | `24,009 allocs/op` | `170 allocs/op` | **`156 allocs/op`** | **$-99.35\%$ allocs** |
| **1,000 Articles** | **Latency** | `4,074.3 μs/op` | `134.7 μs/op` | **`53.3 μs/op`** | **$76.4\times$ faster** |
...
### Key Takeaway
Per-keypress latency for a 1,000-article feed dropped from **$4.07\text{ ms}$** down to **$0.053\text{ ms}$ ($53\text{ μs}$)**. Holding navigation keys no longer causes GC thrashing or SQLite disk lockups.
...
2. **Asynchronous Write-Behind Queue (`internal/db.go`)**:
   ...
   * A background worker debounces writes with a 200ms window / 50-item threshold and executes batched SQLite transactions:
     ...
     ```
   * Added `DB.Close()` with `workerDone` channel synchronization for clean, graceful flushing on application exit.
3. **Preview Lazy Memoization (`internal/article.go` & `internal/window.go`)**:
   * Added `preview string` field to `Article` struct.
   * `AddPreview()` converts HTML to plain text once on first visit and reuses the cached string on subsequent selections.

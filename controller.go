...
func (c *Controller) GetConfigKeys() map[string]string {
	...
	keys["Date Sort"] = c.conf.KeySortByDate
	keys["Feed Sort"] = c.conf.KeySortByFeed
	keys["Title Sort"] = c.conf.KeySortByTitle
	keys["Unread Sort"] = c.conf.KeySortByUnread
...
func (c *Controller) GetArticlesFromDB() {
	...
	log.Printf("Loaded articles from DB: %v", len(c.articles))
}
...
func (c *Controller) ShowFeeds() {
	...
	c.win.AddToFeeds("[lightred]Unread", urTotal, urTotal, &Article{feed: "unread"})

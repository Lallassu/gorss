...
func (w *Window) SetupWindow() {
	...
	w.flexMiddle.AddItem(w.articles, 0, 2, false)
	w.flexMiddle.AddItem(w.preview, 0, 1, false)

	w.flexFeeds = tview.NewFlex().SetDirection(tview.FlexRow)
	w.flexFeeds.SetBackgroundColor(tcell.GetColor("#FF0000"))
	w.flexFeeds.AddItem(w.feeds, 0, 1, false)

	w.flexGlobal = tview.NewFlex().SetDirection(tview.FlexColumn)
	w.flexGlobal.AddItem(w.flexFeeds, 0, 1, false)
	w.flexGlobal.AddItem(w.flexMiddle, 0, 5, false)
